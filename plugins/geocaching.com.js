...
GeocachingCom.prototype.loadMapPage = function(params, success, failure){
	var url = 'http://www.geocaching.com/map/default.aspx';
	...
	var ajaxId = 'map-page-'+ Math.round(new Date().getTime());
	Geocaching.ajaxRequests[ajaxId] = new Ajax.Request(url, {
		'method': 'get',
		'onSuccess': function(r){
			Geocaching.lastAjaxId = null;
			...
			var userToken = '';
			try {
				userToken = reply.match(/var userToken = '([^']+)'/)[1]
			} catch(e) {
				failure($L("Error occured in getting user token on map."));
				return false;
			}
	...
	window.setTimeout(function(ajaxId, params, success, failure) {
		if(Ajax.activeRequestCount > 0 && ajaxId == Geocaching.lastAjaxId) {
			...
			if(params['retry']<=1) {
				params['retry']++;
				this.loadMapPage(params, success, failure);
			}
		}
	}.bind(this), timeout*1000, ajaxId, params, success, failure);
};

GeocachingCom.prototype.loadCachesOnMap = function(params, success, failure){
	var url = 'http://www.geocaching.com/map/default.aspx/MapAction';
	var parameters = {
		'dto': {
			'data': {
				'c': 1,
				'm': "",
				'd': params['lat2'] +'|' +params['lat1'] +'|'+ params['lon2'] +'|'+ params['lon1']
			},
			'ut': params['token']
		}
	};
	...
	var ajaxId = 'map-'+ params['lat2'] +'-'+ Math.round(new Date().getTime());
	Geocaching.ajaxRequests[ajaxId] = new Ajax.Request(url, {
		'method': 'post',
	...
	window.setTimeout(function(ajaxId, params, success, failure) {
		if(Ajax.activeRequestCount > 0 && ajaxId == Geocaching.lastAjaxId) {
			...
			if(params['retry']<=1) {
				params['retry']++;
				this.loadCachesOnMap(params, success, failure);
			}
		}
	}.bind(this), timeout*1000, ajaxId, params, success, failure);
};

GeocachingCom.prototype.loadCacheDetailOnMap = function(params, success, failure){
	var url = 'http://www.geocaching.com/map/default.aspx/MapAction';
	var parameters = {
		'dto': {
			'data': {
	...
	var ajaxId = 'map-'+ params['id'] +'-'+ Math.round(new Date().getTime());
	Geocaching.ajaxRequests[ajaxId] = new Ajax.Request(url, {
		'method': 'post',
		'postBody': Object.toJSON(parameters),
		'contentType': 'application/json',
		...
		'onSuccess': function(r){
			...
			delete(Geocaching.ajaxRequests[ajaxId]);
			var reply = r.responseJSON;	
			var cache = reply['d'].evalJSON();

			success(cache['cs']);
		}.bind(this),
		'onFailure': function(r){
			failure($L("Error occured in getting cache detail."));
		},
		'onException': function(r){
			failure($L("Error occured in getting cache detail."));
		}
	...
	window.setTimeout(function(ajaxId, params, success, failure) {
		if(Ajax.activeRequestCount > 0 && ajaxId == Geocaching.lastAjaxId) {
			...
			if(params['retry']<=1) {
				params['retry']++;
				this.loadCacheDetailOnMap(params, success, failure);
			}
		}
	}.bind(this), timeout*1000, ajaxId, params, success, failure);
};
